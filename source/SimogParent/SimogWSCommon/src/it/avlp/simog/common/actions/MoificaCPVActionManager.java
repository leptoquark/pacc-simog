...
public class MoificaCPVActionManager {
	 ...
	 * @return
	 */	
	public synchronized static ResponseModificaCPV execute(String ticket, String indexCollaborazione,
	      String cig, String cpvPrevalente, List<CpvLotto> cpvSecondaria){
		//-------	object declarations		-------//
		ResponseModificaCPV rgc = null;
		Logger logger = null;
		...
		try{
			...
			if(wss != null){
				...
				try{
					tm.validateRequestedActionByProfile(wss, TicketManager.MODIFICA_CPV);
					if(tm.isValido()){
...
//							}
						...
						try{
							...
							Lotto lottoCPV = new Lotto();
							if (cpvPrevalente != null) {
								lottoCPV.setId_CPV(cpvPrevalente);
							}
							boolean esito = false;
							if (cpvSecondaria != null && !cpvSecondaria.isEmpty()) {
								// validazione dati ed integrazione sul db 
								esito = garaLottoManager.ModificaCPV(lottoCPV, wss.getUserId(),collsRssa,cig, tm.isOperaComeOsservatorio(), cpvSecondaria);
							}else {
								// validazione dati ed integrazione sul db 
								esito = garaLottoManager.ModificaCPV(lottoCPV, wss.getUserId(),collsRssa,cig, tm.isOperaComeOsservatorio(), null);
							}
