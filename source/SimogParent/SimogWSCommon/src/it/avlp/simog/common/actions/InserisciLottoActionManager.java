...
import it.avlp.simog.common.beans.ResponseInserisciLotto;
...
public class InserisciLottoActionManager {
	
	/****************************************************************
	 * Il metodo permette di inserire i dati relativi al Lotto
	 * @param ticket
	 ...
	 * @return
	 */
	public synchronized static ResponseInserisciLotto execute(String ticket, String indexCollaborazione,String datiGara,String id_gara){
	//-------	object declarations		-------//
	ResponseInserisciLotto rgc = null;
	Logger logger = null;
	...
	try{
		...
		rgc=new ResponseInserisciLotto();
		...
		if(wss != null){
			...
			try{
				tm.validateRequestedActionByProfile(wss,TicketManager.INSERISCI_LOTTO);
				if(tm.isValido()){
					...
					try{
						...
						CIGBean cig = garaLottoManager.inserisciLotto(lotto, wss.getUserId(),collsRssa,id_gara, tm.getAdminOr(), tm.isOperaComeOsservatorio());
                  ...
                  if(SimogFlags.is3031_RFWEBGL02Active()
                     ...
                     if(garaLottoManager.getCuplotto()!=null)
						...
						//altrimenti
//						VL : if( cig != null && !tm.isOsservatorioRegionale()){
						if( cig != null ){
							rgc.setSuccess(true);
							rgc.setCig(cig);
						...
						logger.error("inserimento fallito "+swe.getMyMessage());
						if(cwsm != null){
							cwsm.rollback();
						}
