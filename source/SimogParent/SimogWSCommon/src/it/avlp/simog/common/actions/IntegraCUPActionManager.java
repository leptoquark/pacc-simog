...
public class IntegraCUPActionManager {
	 ...
	 * @return
	 */	
	public synchronized static ResponseIntegraCUP execute(String ticket, String indexCollaborazione,
	      String cig, String flagCUP, TIPIAPPALTO tIPIAPPALTOL, TIPIAPPALTO tIPIAPPALTOFS, 
	      CUPLOTTO cUPLOTTO){
		//-------	object declarations		-------//
	   ResponseIntegraCUP rgc = null;
		Logger logger = null;
		...
		try{
			...
			if(wss != null){
				...
				try{
					tm.validateRequestedActionByProfile(wss, TicketManager.INTEGRA_CUP);
					if(tm.isValido()){
...
//							}
                     ...
                     if (tIPIAPPALTOFS != null){
                        ...
                        lottoCup.setElencoTipoAppaltoLottoF(tipi);
							...
							boolean esito = garaLottoManager.integraDatiCUP(lottoCup, wss.getUserId(),collsRssa,cig, tm.isOperaComeOsservatorio());
	                  ...
	                  if(esito){
							...
							logger.info("integrazione cup fallita "+swe.getMyMessage());
							if(cwsm != null){
								cwsm.rollback();
							}
							rgc.setSuccess(false);
