...
import it.avlp.simog.common.beans.ResponseIntegraCUP;
import it.avlp.simog.common.beans.ResponseIntegraPariOpportunita;
...
import it.avlp.simog.ws.massload.xmlbeans.FlagSNQType;
import it.avlp.simog.ws.massload.xmlbeans.FlagSNType;
import it.avlp.simog.ws.massload.xmlbeans.MisuraPremialeType;
import it.avlp.simog.ws.massload.xmlbeans.MotivoDerogaType;
import it.avlp.simog.ws.massload.xmlbeans.PercentualeType;
...
public class IntegraPariOpportunitaActionManager {
	
	/***********************************************************************************
	 * Esegue la integrazione dei dati CUP del Lotto
	 * @param ticket : String
	 * @param indexCollaborazione : String
	 * @param datiGara : String
	 * @param cig :String
	 * @param tIPIAPPALTOL.getTIPOAPPALTO()
	 * @param tIPIAPPALTOFS.ge 
	 * @param cUPLOTTO 
	 * @return
	 */	
	public synchronized static ResponseIntegraPariOpportunita execute(String ticket, String indexCollaborazione,
	      String cig, FlagSNType flagPnrrPnc, FlagSNQType flagPrevisioneQuota, BigDecimal quotaFemminile, BigDecimal quotaGiovanile, FlagSNType flagMisurePremiali, List<MotivoDerogaBean> MotivoDeroga, List<MisuraPremialeBean> MisuraPremiale){
		//-------	object declarations		-------//
		ResponseIntegraPariOpportunita rgc = null;
		Logger logger = null;
		...
		try{
			...
			rgc=new ResponseIntegraPariOpportunita();
			...
			wss.setComando("IntegraPariOpportunita");
			...
			if(wss != null){
				...
				try{
					tm.validateRequestedActionByProfile(wss, TicketManager.INTEGRA_PARI_OPPORTUNITA);
					if(tm.isValido()){
...
//							}
						...
						try{
							...
							// funzione non attiva! TODO
//						   if(!SimogFlags.is3031_RFWEBGL02Active() || (SimogFlags.is3031_RFWEBGL02Active() && !ConfigurationManager.getInstance().getSimogProperties().isCUPAttivo()))
//						      throw new SimogWSException("ERRORE: " + Messaggi.SIMOG_VALIDAZIONE_000);
//						      
//
//							Lotto lottoCup = new Lotto();
//							lottoCup.setFLAG_CUP(flagCUP);
//
//							if(cUPLOTTO != null){
//   							List<CupLottoAggExt> cups = new ArrayList<CupLottoAggExt>();
//   		               for (CodiciCup elem :  cUPLOTTO.getCODICICUP()){
//   		                  CupLottoAggExt beanCup = new CupLottoAggExt();
//   		                  beanCup.setCup(elem.getCUP());
//   	                     beanCup.setOkUtente(elem.getOK_UTENTE());
//   		                  
//   		                  cups.add(beanCup);
//   		               }
//   
//   							lottoCup.setElencoCup(cups);
//							}
//							
//							if (tIPIAPPALTOL != null){
//   		               List<TipoAppaltoAggBean> tipi = new ArrayList<TipoAppaltoAggBean>();
//   		                for (String elem :tIPIAPPALTOL.getTIPOAPPALTO()) {
//   		                  tipi.add(new TipoAppaltoAggBean(elem));
//   		               }
//   		               lottoCup.setElencoTipoAppaltoLottoL(tipi);
//							}
//							
//                     if (tIPIAPPALTOFS != null){
//                        List<TipoAppaltoAggBean> tipi = new ArrayList<TipoAppaltoAggBean>();
//                         for (String elem :tIPIAPPALTOFS.getTIPOAPPALTO()) {
//                           tipi.add(new TipoAppaltoAggBean(elem));
//                        }
//                        lottoCup.setElencoTipoAppaltoLottoF(tipi);
//                     }

							Lotto lottoPariOpportunita = new Lotto();
							if (flagPnrrPnc != null) {
								lottoPariOpportunita.setFLAG_PNRR_PNC(flagPnrrPnc.toString());
							}
							if (flagPrevisioneQuota != null) {
								lottoPariOpportunita.setFLAG_PREVISIONE_QUOTA(flagPrevisioneQuota.toString());
							}
							if (quotaFemminile != null) {
								lottoPariOpportunita.setQuotaFemminile(quotaFemminile);
							}
							if (quotaGiovanile != null) {
								lottoPariOpportunita.setQuotaGiovanile(quotaGiovanile);
							}
							if (flagMisurePremiali != null) {
								lottoPariOpportunita.setFLAG_MISURE_PREMIALI(flagMisurePremiali.toString());
							}
							if (MotivoDeroga != null && !MotivoDeroga.isEmpty()) {
								lottoPariOpportunita.setElencoMotivoDeroga(MotivoDeroga);
							}
							if (MisuraPremiale != null && !MisuraPremiale.isEmpty()) {
								lottoPariOpportunita.setElencoMisurePremiali(MisuraPremiale);
							}
							...
							boolean esito = garaLottoManager.integraDatiPariOpportunita(lottoPariOpportunita, wss.getUserId(),collsRssa,cig, tm.isOperaComeOsservatorio());
