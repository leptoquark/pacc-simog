...
public class EsclusoXMLManager {
	...
	public EsclusoXMLManager(Connection con,Logger logger,AggiudicazioneBean aggiudicazioneBean,SchedaCompletaType schedaCompletaType,
	 ...
	 */
	public void getEscluso(String tipoEnte) throws SimogWSException{
		try{
			...
			SchedaEsclusoType esclusoType = schedaCompletaType.addNewEscluso();
			/* oggetti con cardinalita' singola	(aggiudicazioni)*/
				/*REQUIRED	appalto	*/
			this.setAppalto(aggiudicazioneBean, esclusoType);
			//XX-X: CUI 
			...
			if(!listOfAggiudicatari.isEmpty()){
				...
				this.setAggiudicatari(aggiudicatariIterator, esclusoType);
			}
			
			/*OPTIONAL	Incaricati responsabili	[0,N]*/
			List<ResponsabileBean> listOfIncaricati = responsabileManager.loadMany(aggiudicazioneBean.getIdAggiudicazione(), aggiudicazioneBean.getDataInizioAggiudicazione(), PSBD.SEZIONE_RE, false);
			if(!listOfIncaricati.isEmpty()){
				Iterator<ResponsabileBean> incaricatiIterator = listOfIncaricati.iterator();
				this.setIncaricati(incaricatiIterator, esclusoType,PSBD.SEZIONE_RE);
			}
		}catch(Exception e){
	 ...
	 * param et SchedaEsclusoType 
	 */
	private void setAppalto(AggiudicazioneBean aggiudicazioneBean,SchedaEsclusoType esclusoType){
		logger.debug("eseguendo: void setAppalto(AggiudicazioneBean abb,SchedaEsclusoType et)");
		//logger.debug(ObjectIntrospector.propertiesInfo(abb.getClass(), abb));
		SottoEsclusoType appalto = esclusoType.addNewAppalto();
	 ...
	 * param et SchedaEsclusoType 
	 */
	private void setAggiudicatari(Iterator<AggiudicatarioBean> aggiudicatariIterator,SchedaEsclusoType esclusoType){
		logger.debug("eseguendo: void setAggiudicatari(Iterator<AggiudicatarioBean> i1,SchedaEsclusoType et)");
...
//					logger.debug("[stato null || \"IT\"] - Flag esteri: "+FlagSNType.Enum.forString("N"));
			...
			SoggAggiudicatarioType sat = esclusoType.addNewAggiudicatari();
	 ...
	 * param SchedaEsclusoType: et
	 * param String : sezione
	 */
	private void setIncaricati(Iterator<ResponsabileBean> responsabiliIterator,SchedaEsclusoType esclusoType, String sezione){
		logger.debug("eseguendo: void setIncaricati(Iterator<ResponsabileBean> i6,SchedaEsclusoType et, String sezione)");
		while(responsabiliIterator.hasNext()){
			...
			IncaricatoType it = esclusoType.addNewIncaricati();
