 ...
 *
 */
 public class SrvRubricaRaggruppamento extends ServletBase implements ParametriServlet{
			...
			if ("menu".equals(request.getParameter("from"))){
	 		...
	 		if ( checkSession(request) ) {
	 			if ( true) {				
	 				...
	 				if(PSBD.TAB_SUBAPPALTO.equals(tab))
					   forward("scheda1/"+ParametriServletRubrica.JSP_RUBRICA_DITTA_RAGGRUPPAMENTO_POPUP_SUB, request, response);
	 				else
	 				   forward("scheda1/"+ParametriServletRubrica.JSP_RUBRICA_DITTA_RAGGRUPPAMENTO_POPUP, request, response);
