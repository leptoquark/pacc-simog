package it.avlp.simog.gestionecancellazionemanager.app;
...
import it.avlp.simog.common.servlet.ParametriServletRichCancellazione;
 ...
 *
 */
 public class SrvGestioneRichiestaCancellazione extends ServletBase {
	...
	public SrvGestioneRichiestaCancellazione() {
		super();
	}   	
		...
		if ( checkSession(request) ) {
			if ( currentUser.isAmministratore() || currentUser.isOssReg() || currentUser.isRASA()){
				...
				if (switchOperation(request,"view") || switchOperation(request, "Torna") ||switchOperation(request,"Visualizza") ){
 					logger.debug("Richieste cancellazione - il parametro d'ingresso e' parameter = view" );
 					logger.debug("Passo nell'if VIEW");
 					performView(request,response);
				}else if (switchOperation(request, "viewDetail")){
 					logger.debug("SrvGestioneRichiestaCancellazione - l'operazione e' viewDetail e il parametro d'ingresso e' parameter = idRichiesta");
 					performViewDetail(request, response);
				}else if (switchOperation(request, "Salva")){
	 				logger.debug("SrvGestioneRichiestaCancellazione - l'operazione e' Salva *****************" );
	 			
	 				
	 				performSave(request, response);
	 			
	 			}else if (switchOperation(request, "viewChoose")){
	 				logger.debug("SrvGestioneRichiestaCancellazione - l'operazione e' viewChoose *****************" );
	...
	public void performSave(HttpServletRequest request, HttpServletResponse response)
 	throws ServletException, IOException {		
		...
		try{
			...
			String blocco = request.getParameter(ParametriServletRichCancellazione.FIELD_NAME_BLOCCO);
			...
			boolean result = annAction.gestisciRichiestaCancellazione(richAnnullamento,currentUser.getLogin());
	
			commit(currentActiveConnection);
			setMessage ( request, Messaggi.SIMOG_RICHIESTA_CANCELLAZIONE_002);
			forward("richiesteCancellazione?"+it.avlp.simog.common.servlet.ParametriServletRichCancellazione.OPERAZIONE+"=view&"+ParametriServletRichCancellazione.FIELD_NAME_SCELTA_LISTA+"="+ParametriServletRichCancellazione.FIELD_NAME_SCELTA_LISTA_DA_VALUTARE, request, response);
				...
				sendError(request, response, sqle.getMessage(), ParametriServletRichCancellazione.JSP_DETTAGLIO_RICHIESTE_CANCELLAZIONE, sqle);
			}catch(ClassNotFoundException cnfe){
				rollback(currentActiveConnection);
				sendError(request, response, cnfe.getMessage(), ParametriServletRichCancellazione.JSP_DETTAGLIO_RICHIESTE_CANCELLAZIONE, cnfe);
			}catch(Exception e){
				rollback(currentActiveConnection);
				sendError(request, response, e.getMessage(), ParametriServletRichCancellazione.JSP_DETTAGLIO_RICHIESTE_CANCELLAZIONE, e);
			}finally {
	...
	public void performViewChoose(HttpServletRequest request, HttpServletResponse response)
 	throws ServletException, IOException{
		...
		visualizzaListaParametriValori(request, response);
		forward(ParametriServletRichCancellazione.JSP_RICHIESTE_CANCELLAZIONE, request, response);
	...
	public void performView(HttpServletRequest request, HttpServletResponse response)
 	throws ServletException, IOException {
		...
		try {
			...
			String scelta = request.getParameter(ParametriServletRichCancellazione.FIELD_NAME_SCELTA_LISTA);
			
			if(scelta==null || "".equals(scelta))
				scelta = (String) request.getAttribute(ParametriServletRichCancellazione.FIELD_NAME_SCELTA_LISTA);
			...
			if(cig_lotto!=null && !"".equals(cig_lotto) && cig_lotto.length() != 10){
				sendError(request, response, Messaggi.SIMOG_GARA_010, ParametriServletRichCancellazione.JSP_RICHIESTE_CANCELLAZIONE);
				return;
			}
			...
			TableBean richiesteCancellazione = new TableBean();
			richiesteCancellazione = annAction.getRichiesteAnnullamento(scelta, cig_lotto, currentUser.getAdminOr(), orderField,startRow,configuration.getPaginazioneLogAndRichA(), true); 
			if ( richiesteCancellazione.size() == 0 ) {
				sendMessage(request, response, Messaggi.SIMOG_RIC_001,ParametriServletRichCancellazione.JSP_RICHIESTE_CANCELLAZIONE);
			} else {
				request.setAttribute(ParametriServlet.TABLEBEAN, richiesteCancellazione);
				/**----------------------------*/
				...
				request.setAttribute(ParametriServletRichCancellazione.FIELD_NAME_SCELTA_LISTA,scelta);
				request.setAttribute(ParametriServletRichCancellazione.FIELD_NAME_FILTRO_CIG ,cig_lotto);
				request.setAttribute(ParametriServlet.ORDER_FIELD,orderField);
				/**----------------------------*/
				forward(ParametriServletRichCancellazione.JSP_RICHIESTE_CANCELLAZIONE, request, response);
			}
		} catch ( Exception sqle ) {
			sendError(request, response, sqle.getMessage(), ParametriServletRichCancellazione.JSP_RICHIESTE_CANCELLAZIONE, sqle);
		}finally {
			closeConnection(request.getSession().getId(),getClass().getName());
		}
	}
	...
	public void performViewDetail(HttpServletRequest request, HttpServletResponse response)
 	throws ServletException, IOException {
		...
		try {
			idRichiesta = Integer.parseInt(request.getParameter(ParametriServletRichCancellazione.FIELD_NAME_ID_RICHIESTA));
			currentActiveConnection = getSimogConnection(request.getSession().getId(),getClass().getName());
			BaseRichiestaAnnullamento annAction = AnnullamentoFactory.getAction(null, currentActiveConnection, logger);
			...
			if ( dettaglioRicAnnullamento.size() == 0 ) {
				...
				request.setAttribute(ParametriServletRichCancellazione.DETTAGLIO_TABLEBEAN, dettaglioRicAnnullamento);
				forward(ParametriServletRichCancellazione.JSP_DETTAGLIO_RICHIESTE_CANCELLAZIONE, request, response);
			}
		} catch ( Exception sqle ) {
			sendError(request, response, sqle.getMessage(), ParametriServletRichCancellazione.JSP_RICHIESTE_CANCELLAZIONE, sqle);
		} finally {
	...
	private boolean checkValueParameter(HttpServletRequest request, String parameter, String label) throws Exception{
		...
		if (parameter.equals(ParametriServletRichCancellazione.FIELD_NAME_SCELTA_LISTA) && strParam==null ) {
			check = false;
			throw new Exception(Messaggi.SIMOG_RICHIESTA_CANCELLAZIONE_003);
		}
		if (parameter.equals(ParametriServletRichCancellazione.FIELD_NAME_ESITO) && strParam==null ) {
			check = false;
			throw new Exception(Messaggi.SIMOG_RICHIESTA_CANCELLAZIONE_003);
		}
		return check;
