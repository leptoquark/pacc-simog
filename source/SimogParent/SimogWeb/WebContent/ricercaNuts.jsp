...
<!--<link type="text/css" rel="stylesheet" href="xtree/xtree2.links.css" />-->
<script type="text/javascript">

function selectRow(row){
    var dest =  document.getElementById('NUTS_ID');
...
<title><utils:message key="ricerca.ricercaNUTS" /></title>
<base target="_self">
</head>

<body onload="search('sel_NUTS','NUTS_ID');">

	<form id="IdFormPopup" action="<%=ParametriServlet.SRV_RICERCA_NUTS%>"  method="post">
	<div class="bodypage-e">
	<fieldset style="margin: 2px;padding: 2px;" > 
	<fieldset style="margin: 2px;padding: 2px">
	<legend><utils:message key="ricerca.elencoNUTS" /></legend>
	...
	<script type="text/javascript">
			...
			var tree = new WebFXLoadTree("NUTS", "<%= request.getContextPath() %>/ricercaNuts?idNuts=1");  
			tree.write();
			//document.write(tree);
		</script>
		</div>
			...
			<fieldset style="margin: 2px;padding: 2px">
				<legend><utils:message key="ricerca.ricercaNUTS" /></legend>
				<div>
					<table>
					<tr>	
			        <input type="hidden" name="idNuts" value="1"  id=""/>
					 ...
					 <jsp:useBean id="NUTS_LIST" scope="session" class="java.util.LinkedList" type="java.util.LinkedList"/>
					 <tr>  
						<td align="left">
						<div style="overflow: auto;height: 155px;width: 565px;margin: 0px;padding: 0px">
						<table width="100%">
						<c:forEach items="${NUTS_LIST}" var="nuts">
						   <tr  onclick="selectRow(this);" id="<c:out value='${nuts.idNuts}'/>">	 
							   <td width="15%"><c:out value="${nuts.idNuts}"></c:out> </td>
							   <td> <c:out value="${nuts.descrizione}"></c:out></td>
						   </tr>
						</c:forEach>
						
						</table></div>
						<%session.removeAttribute("NUTS_LIST"); %>
						</td>
					</tr>
	...
	<div align="left" style="margin: 2px;padding: 2px">
		<utils:message key="ricerca.nutsSelezionato" />: <input type="text" name="NUTS_NAME" value="" maxlength="10" id="NUTS_ID" readonly="readonly"/>
	<input type="button" value="<utils:message key="button.conferma" plain="true" />" onclick="setRetVal('NUTS_ID')" />
