...
<%@page import="it.avlp.simog.validatore.WorkFlowController"%>
<%@page import="it.avlp.simog.beans.IdentificativoSchede"%>
<title><utils:message key="richiesta.richiestaCancellazione" /></title>
</head>
...
if(IdentificativoSchede.TAB_INFO_COMUNI.equals(tab))
	...
	String valore = (String) request.getAttribute(PSBD.NOME_SCHEDA) ;	
%>
...
<!--	
	...
	function richiediCancellazione1(tabName, idMotivazione, scelta) {
	   var motivazione = document.forms[0].elements[idMotivazione].value;
	    ...
	    if (checkMotivazione == false || (trim(motivazione).length >= 10 && trim(motivazione).length <= 1000)) {
	        if (scelta == "S") {
	            if (!(typeof i18n !== 'undefined' && i18n.confirm ? i18n.confirm('error.confirmDeleteRequest') : confirm("Confermare richiesta di eliminazione completa"))) {
	                return;
	            }
	        }
	        document.forms[0].elements['<%=PSBD.TAB%>'].value = tabName;
	        document.forms[0].elements['<%=PSBD.ACTION_TYPE%>'].value = "<%=PSBD.ACTION_RICHIESTA_CANCELLAZIONE%>";
	        ...
	        alert('<%= MessageHelper.getMessage(request, "richiesta.motivazioneLunghezza") %>');
	    }
	}	
// --> 
</SCRIPT>
	...
	<div id="gabbia">
		...
		<div id="bodypage">
			<div class="bodypage-e">
					...
					<input type="hidden" name="toDo"  value="<%=PSBD.ACTION_RICHIESTA_CANCELLAZIONE %>">
					...
					<h1>Gestione Schede - Richiesta Di Cancellazione</h1>
...
<%} %>
								...
								<tr>
									...
									<td>
										<textarea rows="5" cols="30" name="<%= PSBD.MOTIVAZIONE_CANCELLAZIONE %>" id="<%= PSBD.MOTIVAZIONE_CANCELLAZIONE %>"></textarea>
									</td>
								</tr>
							</table>
							
							<% if (false){ 
								//FIXME: PP da abilitare quando pronto il metodo che la gestisce 
								//	&& valore.equals(WorkFlowController.AGGIUDICAZIONE_VN)) { 
									%>
							
								<label for="">Eliminazione completa ?</label> &nbsp;
						    						    	
						    	<select id="<%= PSBD.CANC_COMPLETA %>" name="<%= PSBD.CANC_COMPLETA %>" >
					   		...
					   		<input type="hidden" id="<%= PSBD.CANC_COMPLETA %>" name="<%= PSBD.CANC_COMPLETA %>" value="N">
					   			
					   		<%	}	%>
						</fieldset>
						
						<input type="hidden" name="Modificato" value="0">
						<input type="button" value="Salva" onclick="richiediCancellazione1('<%= tab %>','<%= PSBD.MOTIVAZIONE_CANCELLAZIONE %>',document.forms[0].<%= PSBD.CANC_COMPLETA %>.value)" >
						<% String link = "torna()"; %>					
						<% if(tab.equals(IdentificativoSchede.TAB_INFO_COMUNI)){ %>
