...
<%	
	...
	TableBean statoAvanzamento = (request.getAttribute(ParametriServletSchedaB2.STATO_AVANZAMENTO_TABLEBEAN)) != null ? (TableBean)request.getAttribute(ParametriServletSchedaB2.STATO_AVANZAMENTO_TABLEBEAN): new TableBean();
	TableBeanRow rowStatoAvanzamento = statoAvanzamento.getRow(0);
	TableBean motivazioniSospensione = (request.getAttribute(ParametriServletSchedaB2.MOTIVI_SOSPENSIONE_TABLEBEAN)) != null ? (TableBean)request.getAttribute(ParametriServletSchedaB2.MOTIVI_SOSPENSIONE_TABLEBEAN): null;
	TableBean selMotivazioniSospensione = (request.getAttribute(ParametriServletSchedaB2.MOTIVI_SOSPENSIONE_SELECTED_TABLEBEAN)) != null ? (TableBean)request.getAttribute(ParametriServletSchedaB2.MOTIVI_SOSPENSIONE_SELECTED_TABLEBEAN): null;
	TableBean motivazioniVariante = (request.getAttribute(ParametriServletSchedaB2.MOTIVI_VARIANTE_TABLEBEAN)) != null ? (TableBean)request.getAttribute(ParametriServletSchedaB2.MOTIVI_VARIANTE_TABLEBEAN): null;
	TableBean selMotivazioniVariante = (request.getAttribute(ParametriServletSchedaB2.MOTIVI_VARIANTE_SELECTED_TABLEBEAN)) != null ? (TableBean)request.getAttribute(ParametriServletSchedaB2.MOTIVI_VARIANTE_SELECTED_TABLEBEAN): null;
	TableBean certificatiPagamento = (request.getAttribute(ParametriServletSchedaB2.CERTIFICATI_PAGAMENTO_TABLEBEAN)) != null ? (TableBean)request.getAttribute(ParametriServletSchedaB2.CERTIFICATI_PAGAMENTO_TABLEBEAN): null;
	TableBean attiAggiuntivi = (request.getAttribute(ParametriServletSchedaB2.ATTI_AGGIUNTIVI_TABLEBEAN)) != null ? (TableBean)request.getAttribute(ParametriServletSchedaB2.ATTI_AGGIUNTIVI_TABLEBEAN): null;
	...
	// CIAO
	String DESCRIZIONE_STATO	= (statoAvanzamento.getFullSize()>0) ? rowStatoAvanzamento.getNulledField(STATI_SCHEDA.DESCRIZIONE) : "";
   ...
   String data_cancellazione_lotto = (infoEnteContratto!=null)? infoEnteContratto.getNulledField(LOTTO.DATA_CANCELLAZIONE_LOTTO,0) : ""; 
    ...
    boolean noOp = false;
	String ID_STATO	= (statoAvanzamento.getFullSize()>0) ? rowStatoAvanzamento.getNulledField(AGGIUDICAZIONE.ID_STATO) : "";
	...
	String modificato3 = (request.getAttribute("Modificato3")!=null)?(String)request.getAttribute("Modificato3"):"0";
	String modificato4 = (request.getAttribute("Modificato4")!=null)?(String)request.getAttribute("Modificato4"):"0";
%>
<html>
	<head>
	<title><utils:message key="scheda.gestioneSchede" /> - <utils:message key="scheda.statiAvanzamento" /> - <%= user.getProfilo() %></title>
	</head>
	<body>
		<div id="gabbia">
			<%@ include file="../include/header.inc" %>			
			<div class="bodypage-e">
				<form id="FormSchedaB2" name="gestioneTab" action="<%= ParametriServletSchedaB2.SRV_SCHEDA_B2 %>" method="post" onkeypress="setFormModified('Modificato0')">
					<h1><utils:message key="scheda.gestioneSchede" /> - <utils:message key="scheda.statiAvanzamento" /></h1>
					...
					<div  class="hmenu">	
						<ul>
							<li><a title="<utils:message key="scheda.tornaListaAggiudicazioni" plain="true" />" href="javascript:changePage('<%=ParametriServlet.SRV_VISUALIZZA_RIEPILOGO_SCHEDA%>','Modificato')"><utils:message key="button.torna" /></a></li>  
						</ul>
					</div>
					...
					<fieldset>
						...
						<table width = "100%">
							...
							<tr>
								...
								<td>&nbsp;</td>
								<td align="center" class="AA<%=TIPO_CONTRATTO%>"><p style="font-weight: bolder; color: black">CONTRATTI DI APPALTO DI IMPORTO &gt; 500.000 EURO </p></td>
							</tr>
							<tr>
							...
							<tr>
								<td align="center" class="AA<%=TIPO_CONTRATTO%>"><font color="black" style="font-weight: bold;">FASE DI ESECUZIONE  E AVANZAMENTO DEL CONTRATTO</font></td>
								<td align="center"  style="font-size: large; font-weight: bold">EA<%= TIPO_CONTRATTO %></td>
					...
					<fieldset>
						...
						<table>	
							<tr>
								<td><input <%=disabled%> type="button" value="Salva" onclick="setAndSave('FormSchedaB2','<%= ParametriServletSchedaB2.TAB_ESECUZIONE_CONTRATTO %>')"></td>
								<td><input <%=disabled%> <%=noconf%> type="button" value="Conferma" onclick="setAndConfirm('FormSchedaB2','<%= ParametriServletSchedaB2.TAB_ESECUZIONE_CONTRATTO  %>')"></td>
								<td><input <%=disabled%> type="button" value="Reimposta" onclick="reimposta('<%= ParametriServletSchedaB2.TAB_ESECUZIONE_CONTRATTO %>')"></td>
								<% if(confermato && !DESCRIZIONE_STATO.contains(PSBD.MSG_RICHIESTO_ANNULLAMENTO)) { %>
									<td><input type="button" value="<%= ParametriServlet.ACTION_RICHIEDI_ANNULLA %>" onclick="loadAnnullamentJSP('<%= ParametriServletSchedaB2.TAB_ESECUZIONE_CONTRATTO  %>')"></td>
								<% } %>
							</tr>
						</table>
						<table width="100%">	
							<tr>
								<td colspan="2">
									<div id="<%= ParametriServletSchedaB2.TAB_ESECUZIONE_CONTRATTO  %>" onkeypress="setFormModified('Modificato0')">
										<%@ include file="datiSchedaB2.inc" %>    
									</div>
								</td>
							</tr>			
						</table>
