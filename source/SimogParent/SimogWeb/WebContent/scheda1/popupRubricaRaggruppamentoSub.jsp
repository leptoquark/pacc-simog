...
function loadDitteRaggruppamento(){
	...
	var idRigaAgg = "hidden<%=PSBD.FIELD_NAME_AGG_LISTA_GRUPPI%>";
	...
	if(table != null){
	    ...
	    for(i=1;i<numrows;i++){
		 ...
		 if(errori == false){ 
	       var padre = getSender(window); //window.opener;
	       var id = "<%= request.getParameter(PSBD.ID_TABELLA_AFFIDATARI) %>";
	       <%if(ParametriServletSubappalti.FIELD_NAME_CF_DITTA == null){  %>	
...
<table>
  <tbody>
   ...
   <% } else{  %>
		 ...
		 <tr> 
		   <td class="garaTd"> <%=request.getParameter( ParametriServletSubappalti.FIELD_NAME_CF_DITTA)%> 
		  </tr> 
	 	<% } %>	
...
<input type="button" value="Torna" onclick="chiudiPopUp('dialogDitteAusiliarie')"></fieldset>
