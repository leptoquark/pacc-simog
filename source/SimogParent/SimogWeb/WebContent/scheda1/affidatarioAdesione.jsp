...
<script type="text/javascript">
  ...
  function setImportoDiAggiudicazione(element, noSup){
	...
	if(document.getElementById('SubTotale').value == null || document.getElementById('SubTotale').value <= 0 ){
		return false;
	}
	subtot = lavori = parseFloat(document.getElementById('SubTotale').value.replace(/\./g,"").replace(',','.'));
	if(document.getElementById('IMPORTO_PROGETTAZIONE').value == ""){
		progettazione = parseFloat('0');
	}else{
		progettazione = parseFloat(document.getElementById('IMPORTO_PROGETTAZIONE').value.replace(/\./g,"").replace(',','.'));
	}
	...
	if(document.getElementById('IMP_NON_ASSOG').value == ""){
		nonAssog = parseFloat('0');
	}else{
		nonAssog = parseFloat(document.getElementById('IMP_NON_ASSOG').value.replace(/\./g,"").replace(',','.'));
	}
	subtot = subtot + progettazione;
	importo = 0;
	if(element.id == 'ribAgg'){
		...
		perc = eval(1.00+perc)
		importo = (parseFloat(subtot)* perc) ;
	}importo = importo + sicurezza + nonAssog;
	...
	function copySubTotale(idField){
		var field = document.getElementById(idField);
		field.value = document.getElementById("SubTotale").value;
	}	
	</script>
	...
	<table>
	 <colgroup>
		...
		<tr>
			<th><label for="">Ribasso di aggiudicazione</label></th>
			<td>
				<input id="ribAgg" tabindex="<%=++indiceTab%>" onblur="calcoloAggiudicazione(this,document.getElementById('percAum'),'<%= PSBD.AGGIUDICATARIO %>')"  readonly style="text-align: right;" type="text" size="9" maxlength="9" name="<%= PSBD.FIELD_NAME_PERC_RIBASSO_AGG %>" value="<c:out value="${aggiudicazione.percRibassoAggStr}" />" onchange="setFormModified('Modificato0')" >
				<input type="hidden" id="<%= PSBD.FIELD_NAME_PERC_RIBASSO_AGG %>" name="<%= PSBD.FIELD_NAME_PERC_RIBASSO_AGG %>" value="<c:out value="${aggiudicazione.percRibassoAggStr}" />"></input>
			
			</td>
		</tr>
			<tr>
				<th><label for=""><utils:message key="scheda.offertaInAumento" /></label></th>
				<td>
					<input id="percAum" tabindex="<%=++indiceTab%>" onblur="calcoloAggiudicazione(document.getElementById('ribAgg'),this,'<%= PSBD.AGGIUDICATARIO %>')"  readonly  style="text-align: right;" type="text" size="9" maxlength="9" name="<%= PSBD.FIELD_NAME_PERC_OFF_AUMENTO %>" value="<c:out value="${aggiudicazione.percOffAumentoStr}" />" onchange="setFormModified('Modificato0')">%
					<input type="hidden" id="<%= PSBD.FIELD_NAME_PERC_OFF_AUMENTO %>" name="<%= PSBD.FIELD_NAME_PERC_OFF_AUMENTO %>" value="<c:out value="${aggiudicazione.percOffAumentoStr}" />"></input>	
				</td>
			</tr>
			<tr>
				...
				<input tabindex="<%=++indiceTab%>" id="euro" type="text" style="text-align:right;font-weight:bold;width: 120px" name="<%= PSBD.FIELD_NAME_IMPORTO_AGGIUDICAZIONE %>"  ${readonlyAffidStr}  value="<c:out value="${aggiudicazione.importoAggiudicazioneStr}" />"
					   onchange="setFormModified('Modificato0')" 
					   onblur="validateAmount(this)" />
			   <input type="hidden" id="<%= PSBD.FIELD_NAME_IMPORTO_AGGIUDICAZIONE %>" name="<%= PSBD.FIELD_NAME_IMPORTO_AGGIUDICAZIONE %>" value="<c:out value="${aggiudicazione.importoAggiudicazioneStr}" />"></input>	
			...
			<tr>			
				<th><label for=""><utils:message key="scheda.dataAggiudicazioneDefinitiva" />*</label></th>
				<td>
					<input tabindex="<%=++indiceTab%>" style="text-align:center" onchange="setFormModified('Modificato0')" onblur="Calendar.validaData(this)" type="text" id="dtaggdef" name="<%= PSBD.FIELD_NAME_DATA_AGGIUDICAZIONE_DEFINITIVA %>" disabled value="<c:out value="${aggiudicazione.dataVerbaleAggiudicazione}" />">
					<input type="hidden" id="<%= PSBD.FIELD_NAME_DATA_AGGIUDICAZIONE_DEFINITIVA %>" name="<%= PSBD.FIELD_NAME_DATA_AGGIUDICAZIONE_DEFINITIVA %>" value="<c:out value="${aggiudicazione.dataVerbaleAggiudicazione}" />"></input>											
				</td>
			</tr>
		   <tr>
			  <th><label> L'affidatario ha richiesto in sede di offerta la possibilita di subappaltare parte delle prestazioni?</label></th>
	  		  <%-- 
	  		  <td width="40%">
	  		  --%>
	  		  <td>
	  			 <input onchange="setFormModified('Modificato0')" tabindex="<%=++indiceTab%>" id="affidatarioY" type="radio" name="<%= PSBD.FIELD_NAME_FLAG_RICH_SUBAPPALTO %>" disabled value="S" <c:out value="${aggiudicazione.flagRichSubappalto == 'S' ? 'checked' : ''}" /> />SI 
 				 <input onchange="setFormModified('Modificato0')" tabindex="<%=++indiceTab%>" id="affidatarioN" type="radio" name="<%= PSBD.FIELD_NAME_FLAG_RICH_SUBAPPALTO %>" disabled value="N" <c:out value="${aggiudicazione.flagRichSubappalto == 'N' ? 'checked' : ''}" /> />NO
 			 	 <input type="hidden" id="<%= PSBD.FIELD_NAME_FLAG_RICH_SUBAPPALTO %>" name="<%= PSBD.FIELD_NAME_FLAG_RICH_SUBAPPALTO %>" value="<c:out value="${aggiudicazione.flagRichSubappalto}" />"></input>			 
 			 </td>
	  	  </tr>			
		</table>
			...
			<table id="idTabella<%= prefixAgg %>">
				<tbody>
					<tr>
						...
						<!-- Rinaldo tiket 654 -->
						<th class="garaTh">Importo Aggiudicazione</th>
						<th class="garaTh">Ribasso di aggiudicazione</th>
						<th class="garaTh">Offerta in aumento</th>
						<!-- ///////////////// -->
						<th class="garaTh">Richiesta Ricorso Avvalimento</th>
						<th class="garaTh">Codice Fiscale Ditta Ausiliaria</th>
						<%-- gm aggiunto per raggruppamenti di impresa --%>
						<th class="garaTh">Progressivo Raggruppamento</th>
					</tr>
					...
					<c:forEach var="aggCorrente" items="${aggiudicatari}">
						...
						<tr id="<c:out value="${id}" />">
							...
							<c:set var="parametri" value="ID_TABELLA_AFFIDATARI=${id}&SOGGETTI_PARTECIPANTIDENOMINAZIONE=${soggPartecipante.denominazionePulitaSl}&CODICE_FISCALE=${soggPartecipante.codiceFiscale}&SOGGETTI_PARTECIPANTIID_STATO=${soggPartecipante.id_stato}&RuoloAgg=${aggCorrente.ruolo}&ID_GRUPPO=${aggCorrente.idGruppo}&ID_TIPOAGG=${aggCorrente.idTipoAgg}&readonlyAffidatario=${readonlyAffid}" />							
							<c:if test="${false}">
							...
							<c:if test="${true}">
								...
								--%>
								<td>
									<input type="checkbox" name="<c:out value="affidatarioSelez_${counter}" />" id="<c:out value="affidatarioSelez_${counter}" />" value="<c:out value="${soggPartecipante.idSoggettoPartecipante}" />" onchange="setFormModified('Modificato0')" <c:out value="${aggCorrente.selezionato == 1 ? 'checked' : ''}" /> ${disabledStr} />
								</td>
							</c:if>
							...
							<td nowrap style="display: none;" id="<c:out value="${id}" /><%= PSBD.FIELD_NAME_AGG_PERCENTUALE %>"><c:out value="${aggCorrente.percentuale}" /></td>
							...
							<td id="<c:out value="${id}" /><%= PSBD.FIELD_NAME_AGG_FLAG_AVVALIMENTO %>" class="garaTd">
							
							
<table>
<c:forEach var="aux" items="${aggCorrente.ditteAusiliarie}">
<tr><td><c:out value="${aux.flagAvvalimentoDecod}" /></td></tr>
</c:forEach>
<%-- <c:out value="${aggCorrente.primoAvvalimento}" /> --%>
</table>
</td>

							<td class="garaTd" id="<c:out value="${id}" /><%= PSBD.FIELD_NAME_AGG_CF_AUSILIARIA %>">
<table>
<c:forEach var="aux2" items="${aggCorrente.ditteAusiliarie}">
<tr><td><c:out value="${aux2.soggettoPartecipante.codiceFiscale}" /></td></tr>
</c:forEach>
</table>
</td>
<%-- <c:out value="${aggCorrente.primoAvvalimento}" /> --%>
...
<%-- <c:out value="${aggCorrente.primaAusiliaria}" /> --%>
		...
		<%	String checkAssCat = (String)request.getAttribute("ASS_CATEGORIA");
			if(request.getAttribute("ASS_CATEGORIA")!=null && !"".equals(checkAssCat)) {%>
			   	<div class="hmenu"><a id="showHide<%= PSBD.AGGIUDICATARIO %>Button" title="Aggiungi <%= prefixAgg %>" href="javascript:showSezioneAggiungiAdesione([<%= PSBD.argsAggiud %>],[<%=PSBD.argsAggiudNascosti%>],'<%=prefixAgg%>')" title="Aggiungi <%= prefixAgg %>">Aggiungi Aggiudicatario</a></div>
		<% } %>
		...
		<div class="detailHelp" id="divAgg<%= prefixAgg %>" style="display: none; border: 1px solid #cfcfcf;">
			...
			<table width="100%">
				<tr>
					<th><label for="">Codice fiscale dell' aggiudicatario o affidatario</label></th>
					<td>
					...
					<td>
						<div class="hmenu"><a title="Cerca in rubrica" href="javascript:apriPopUpRubrica('rubrica','<%= PSBD.TAB_AFFIDATARIO %>','Rubrica Operatore Economico')">Cerca in rubrica</a></div>						
					</td>					
				</tr>
				<tr>
				...
				<tr><td class="hmenu"><a id="AddMod<%= prefixAgg %>" href="javascript:addRowAdesione([<%= PSBD.argsAdesione %>],[<%=PSBD.argsAdesioneNascosti%>],'<%=prefixAgg%>')">Aggiungi</a></td></tr>
			</table>	
