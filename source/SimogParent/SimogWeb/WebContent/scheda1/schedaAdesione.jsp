...
<%@ include file="/script/domUtilsAdesione.js" %> 
<script type="text/javascript" src="script/schedaA.js"></script>

<title><utils:message key="scheda.gestioneSchede" /> - <utils:message key="scheda.faseAdesione" /> - <%=user.getProfilo()%></title>
</head>
   <c:set var="datiGara" value="${sessionScope['dati_gara']}" />
	...
	<c:set var="roByFlusso" value="${datiGara.deleted or rupOk eq false or UTENTE.ossReg eq true or UTENTE.RASA eq true or  schedaA.delegaScheda eq true  or schedaA.riaggiudicata eq true}" />
	...
	<c:set var="noConf" value="${roByFlusso eq true or (schedschedaAaEsclusi.aggiudicazione.idAggiudicazione le 0) || schedaA.aggiudicazione.richAnn eq true or schedaA.aggiudicazione.confirmed}" />
	...
	<c:set var="variazAnagraf" value="${(schedaA.varAnagActive eq true and roByFlusso ne true and schedaA.aggiudicazione.confirmed eq true and variazioniAnagrafiche ne true )}" />
	...
	<% } %>
	<c:set var="garaModoReal" value="${datiGara.ID_MODO_REAL}" />
	
<body> 

<div id="gabbia">

<%-- 	<h1>fromAVCPass: ${fromAVCPass} - origine[${schedaA.aggiudicazione.origine}]</h1> --%>
	...
	<%} %>
		...
		<div class="bodypage-e">
			<form id="FormSchedaAdesione" name="gestioneTab" action="<%=ParametriServlet.SRV_SCHEDA_ADESIONE %>" method="post" onkeypress="setFormModified('Modificato')">
			<h1><utils:message key="scheda.gestioneSchede" /> - <utils:message key="scheda.faseAdesioneAccordoQuadro" /></h1>
			...
			<ul> 		
				...
				<li><a title="<utils:message key="scheda.tornaListaAdesioni" plain="true" />" href="javascript:changePage('<%=riScheda%><c:out value="${datiGara.idLotto}" />','Modificato')"><utils:message key="scheda.listaAdesioni" /></a></li>			    
		...
		<table>	
			<tr>
				<td><input  ${variazioniAnagrafiche eq true ? '' : saveAndResetDisabledStr} type="button" value="Salva" onclick="${saveAction}('FormSchedaAdesione','<%=PSBD.TAB_ADESIONE %>')"></td>
				<td><input ${noConf eq true ? 'disabled' : ''} type="button" value="Conferma" onclick="setAndConfirm('FormSchedaAdesione','<%= PSBD.TAB_ADESIONE %>')"></td>
				<td><input  ${variazioniAnagrafiche eq true ? '' : saveAndResetDisabledStr}  type="button" value="Reimposta" onclick="reimposta('<%= PSBD.TAB_ADESIONE%>')"></td>
				<c:if test="${annullabile}">
					<td><input type="button" value="<%= ParametriServlet.ACTION_RICHIEDI_ANNULLA %>" onclick="loadAnnullamentJSP('<%= PSBD.TAB_ADESIONE %>')"></td>
				</c:if>
				...
				<c:set var="statoann" value="${schedaA.aggiudicazione.richAnn}"/>
