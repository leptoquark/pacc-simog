...
var contAffid=${fn:length(aggiudicatari)};
  function setImportoDiAggiudicazione(element, noSup){
	...
	function modAnagAgg(){
		apriPopUpMod('rubrica','<%= PSBD.TAB_AFFIDATARIO %>',document.getElementById('<%= PSBD.FIELD_NAME_AGG_ID_SOGGETTO_PARTECIPANTE%>'),document.getElementById('<%= PSBD.FIELD_NAME_AGG_DATA_INIZIO_SOGG%>'),'<utils:message key="rubrica.rubricaOperatoriEconomici" />',document.getElementById('<%= PSBD.FIELD_NAME_ANAGOE %>').value);
	}	
	</script>	
	...
	<tr>
			...
			<tr>
				...
				<input ${readonlyAffidStr} tabindex="<%=++indiceTab%>" id="euro" type="text" style="text-align:right;font-weight:bold;width: 120px" name="<%= PSBD.FIELD_NAME_IMPORTO_AGGIUDICAZIONE %>" value="<c:out value="${aggiudicazione.importoAggiudicazioneStr}" />"
			...
			<tr>			
				...
				<td>
					...
					<c:if test="${readonlyAffid ne true && !fromAVCPass}">
						...
						<script type="text/javascript">
						    Calendar.setup({
						        inputField     :    "dtaggdef",     // id of the input field
						        ifFormat       :    "%d/%m/%Y",     // format of the input field
						        button         :    "CALdtaggdef",  // trigger for the calendar (button ID)
				...
				<td>
					...
					<c:if test="${readonlyAffid ne true}">
						...
						<script type="text/javascript">
						    Calendar.setup({
						        inputField     :    "dtstip",     // id of the input field
						        ifFormat       :    "%d/%m/%Y",     // format of the input field
						        button         :    "CALdtstip",  // trigger for the calendar (button ID)
							    align          :    "Tl",           // alignment (defaults to "Bl")
							    singleClick    :    true
						    });
						</script>
			...
			<table id="idTabella<%= prefixAgg %>">
				<tbody>
					<tr>
						...
						<%--gm aggiunto per ditte ausilarie--%>
					...
					<c:forEach var="aggCorrente" items="${aggiudicatari}">
						...
						<!-- ///////////////// --> 	
							...
							<%-- <td id="<c:out value="${id}" /><%= PSBD.FIELD_NAME_AGG_FLAG_AVVALIMENTO %>" class="garaTd"><c:out value="${aggCorrente.flagAvvalimentoDecod}" /></td>--%>
							<%-- <td class="garaTd" id="<c:out value="${id}" /><%= PSBD.FIELD_NAME_AGG_CF_AUSILIARIA %>"><c:out value="${aggCorrente.cfAusiliaria}" /></td>--%>						
							<%-- gm aggiunto per raggruppamenti di impresa 
                        ...
                        <a title="<utils:message key="scheda.gestioneDitteAusiliarie" />" href="javascript:apriPopUpRubricaDittaAusiliaria('<c:out value="${id}" />',[<%= PSBD.argsAggiud %>],[<%=PSBD.argsAggiudNascosti%>],'<%=prefixAgg%>','rubricaDittaAusiliaria','<%= PSBD.TAB_AFFIDATARIO %>','<utils:message key="scheda.gestioneDitteAusiliarie" />','<c:out value="${parametri}" />')"><utils:message key="scheda.gestioneDitteAusiliarie" /></a>						 
							</td>
							</c:if>
							--%>	 
							<%--<c:if test="${readonlyAffid eq true}">
                     ...
                     <td style="display: none;"><input type="hidden" id="hidden<c:out value="${id}" /><%= PSBD.FIELD_NAME_AGG_DENOMINAZIONE %>" name="<c:out value="${id}" /><%= PSBD.FIELD_NAME_AGG_DENOMINAZIONE %>" value="<c:out value="${soggPartecipante.denominazione}" />"></td>	
							...
							<td style="display: none;"><input type="hidden" id="hidden<c:out value="${id}" /><%= PSBD.FIELD_NAME_AGG_PERCENTUALE %>" name="<c:out value="${id}" /><%= PSBD.FIELD_NAME_AGG_PERCENTUALE %>" value="<c:out value="${aggCorrente.percentuale}" />"></td>
						   ...
						   <%-- gm aggiunto per gestione ditte ausiliarie --%>
			...
			<table width="100%">
				<tr>
					...
					<td ${variazioniAnagrafiche eq true ? '' : 'style="display:none;"'}>
						<div class="hmenu"><a title="<utils:message key="scheda.modificaAnagrafica" />" href="javascript:modAnagAgg();"><utils:message key="scheda.modificaAnagrafica" /></a></div></td>
					</td>
					<td>
