...
public class WorkFlowAdesione extends _WorkFlow {
...
/*****************************************************************************************************************/
			case IdentificativoSchede.INDICE_ADESIONE:
				...
				if(!temp){
					this.addError(situazioneAttuale.getStatoAdesione(), IdentificativoSchede.ADESIONE, 
							Messaggi.SIMOG_MASSLOADER_173
							.replace("$1", IdentificativoSchede.DATI_COMUNI)
							.replace("$2", "Inserire")
							.replace("$3", IdentificativoSchede.ADESIONE));
				}
				temp = !situazioneAttuale.getStatoAdesione().isEsistenteDb();
				if(!temp){
					this.addError(situazioneAttuale.getStatoAdesione(), IdentificativoSchede.ADESIONE, 
							Messaggi.SIMOG_MASSLOADER_196);
				}
				...
				esito = controlloSchedaConfermata(situazioneAttuale.getStatoDatiComuni()) 
						&& !situazioneAttuale.getStatoAdesione().isEsistenteDb()
...
/*****************************************************************************************************************/
			case IdentificativoSchede.INDICE_ACCORDO_BONARIO:
				...
				esito = controlloSchedaConfermata(situazioneAttuale.getStatoInizioLavori()) 
...
/*****************************************************************************************************************/
			case IdentificativoSchede.INDICE_COLLAUDO:
				...
				esito = controlloSchedaConfermata(situazioneAttuale.getStatoConclusione())
						&& nonEsisteOConfermata(situazioneAttuale.getStatoAccordi())
...
/*****************************************************************************************************************/
			case IdentificativoSchede.INDICE_FINE_LAVORI:
				...
				if(situazioneAttuale.getStatoDatiComuni().isSettoriSpeciali()){
					esito = schedaEsistenteEConfermata(situazioneAttuale.getStatoAdesione());
					if(!esito){
						String avviso = Messaggi.SIMOG_MASSLOADER_173
						.replace("$1", IdentificativoSchede.ADESIONE 
...
/*****************************************************************************************************************/
			case IdentificativoSchede.INDICE_FASE_INIZIALE:
				 ...
				 */
				temp = controlloSchedaConfermata(situazioneAttuale.getStatoAdesione());
				if(!temp){
				...
				esito = controlloSchedaConfermata(situazioneAttuale.getStatoAdesione())
						&& !situazioneAttuale.getStatoConclusione().isEsistenteDb()
...
/*****************************************************************************************************************/
			...
			default:
				this.addError(situazioneAttuale.getByIdScheda(identificativo.getIndiceScheda()), identificativo.getNomeScheda(), Messaggi.SIMOG_MASSLOADER_207.replace("$1", "Accordo quadro/Convenzione senza successivo confronto competitivo"));
	...
	public boolean isCancellabile(IdentificativoSchede identificativo, SituazioneSchedeAttuale situazioneAttuale){
		...
		switch (switcher) {
			case IdentificativoSchede.INDICE_DATI_COMUNI:
				 ...
				 **/
				esito = !situazioneAttuale.getStatoAdesione().isEsistenteDb();
				addErrore178SeNecessario(!esito, IdentificativoSchede.DATI_COMUNI, 
						IdentificativoSchede.ADESIONE, situazioneAttuale.getStatoDatiComuni());
				break;
			case IdentificativoSchede.INDICE_ADESIONE:
				/**
				 * Adesione:
				...
				if(!esito){
					temp = !situazioneAttuale.getStatoInizioLavori().isEsistenteDb();
					addErrore178SeNecessario(!temp, IdentificativoSchede.ADESIONE, 
							IdentificativoSchede.FASE_INIZIALE,  situazioneAttuale.getStatoAdesione());
	
					temp = !situazioneAttuale.getStatoConclusione().isEsistenteDb();
					addErrore178SeNecessario(!temp, IdentificativoSchede.ADESIONE, 
							IdentificativoSchede.FINE_LAVORI, situazioneAttuale.getStatoAdesione());
	
					temp = !isEsistente(situazioneAttuale.getStatoRitardo());
					addErrore178SeNecessario(!temp, IdentificativoSchede.ADESIONE, 
							IdentificativoSchede.IPOTESI_RECESSO, situazioneAttuale.getStatoAdesione());
	
					temp = !isEsistente(situazioneAttuale.getStatoSubAppalti());
					addErrore178SeNecessario(!temp, IdentificativoSchede.ADESIONE, 
							IdentificativoSchede.SUBAPPALTO, situazioneAttuale.getStatoAdesione());
	
					temp = !isEsistente(situazioneAttuale.getStatoVarianti());
					addErrore178SeNecessario(!temp, IdentificativoSchede.ADESIONE, 
							IdentificativoSchede.VARIANTE, situazioneAttuale.getStatoAdesione());
	...
	public EsitoControlloStatiSchede isNotFlussoInRichiesta(
		...
		if(identificativo != null){
			...
			if(identificativo.getIndiceScheda() == IdentificativoSchede.getAdesione().getIndiceScheda()){			
				if(!situazioneAttuale.getStatoAdesione().isEsistenteDb()){
					esitoStati.setEsitoOperazione(false);
					situazioneAttuale.getStatoAdesione().setCig(cig);
					situazioneAttuale.getStatoAdesione().setCui(cui);
					esitoStati.addListOfValidationBeans(EsitoControlloStatiSchede.getValidationBean(Messaggi.SIMOG_MASSLOADER_186, IdentificativoSchede.ADESIONE, situazioneAttuale.getStatoAdesione()));
		...
		if(situazioneAttuale.getStatoAdesione().isEsistenteDb()){
			...
			if(situazioneAttuale.getStatoAdesione().isInRichiestaAnnullamento()){
				esito = false;
				esitoStati.addListOfValidationBeans(EsitoControlloStatiSchede.getValidationBeanInRichAnn(IdentificativoSchede.ADESIONE, situazioneAttuale.getStatoAdesione()));
			}
			if(situazioneAttuale.getStatoAdesione().isInRichiestaCancellazione()){
				esito = false;
				esitoStati.addListOfValidationBeans(EsitoControlloStatiSchede.getValidationBeanInCanc(IdentificativoSchede.ADESIONE, situazioneAttuale.getStatoAdesione()));
			}
	...
	public EsitoControlloStatiSchede isNotFlussoInDefinizione(SituazioneSchedeAttuale situazioneAttuale){
		...
		/** Aggiudicazione **/
		if(situazioneAttuale.getStatoAdesione().isEsistenteDb())
			if(!situazioneAttuale.getStatoAdesione().isConfermata()){
				esito = false;
				esitoStati.addListOfValidationBeans(EsitoControlloStatiSchede.getValidationBeanInDefinizione(IdentificativoSchede.ADESIONE, situazioneAttuale.getStatoAdesione()));
			}
