...
public class EsclusiValidator extends SimogValidator {

	public EsclusiValidator(Connection connection, Logger logger) {
	...
	public boolean valida(Object bean, String section) {
		try{
			...
			if(IdentificativoSchede.TAB_INFO_COMUNI.equalsIgnoreCase(section))
				...
				if(section == null) 
					validaDatiComuni(saBean.getInfoComuni(),saBean.getInfoGara().getImportoLotto().floatValue());
				validaAggiudicazione(saBean.getAggiudicazione(),saBean.getCondizioni(), saBean.getInfoComuni(),saBean.getInfoGara().getID_MODO_REAL(), saBean.getInfoGara().getIdOsservatorio());
				...
				// valido solo se è attiva la funzione, 
				// è passata la data inizio attivazione web o quella di attivazione ws
				// la competenza è dell'aggiudicazione
            if( SimogFlags.is3031_RFWEBGL02Active() 
               ...
               Gara g = gm.getGara(saBean.getInfoGara().getIdGara());
               validaFlageCodiciCUP(lotto,g.getID_MODO_REAL(),g.getID_SVOLGIMENTO());//Ticket ALM #666
            }
            
            
          //TICKET ALM #9272 - 3.04.4
			//Controllo di data quality per le aggiudicazioni con piu' aggiudicatari: la somma degli importi dei singoli aggiudicatari
			...
			if(saBean.getAggiudicatari().size()>1) {
				...
				for(AggiudicatarioBean ab : saBean.getAggiudicatari()) {
					if(ab.getIdTipoAgg()==Costanti.TIPODITTA_SINGOLA || ab.getIdTipoAgg() == Costanti.TIPODITTA_ASS_CAT ||
							   ab.getIdTipoAgg()==Costanti.TIPODITTA_GEIE || 
			...
			if(verificaImportiAgg && saBean.getAggiudicatari().size()>1) {
				...
				if(!SimogFlags.isAccordoQuadroOrConvenzione(gara.getID_MODO_REAL()) && !SimogFlags.isSvolgimentoAccordoQuadro(gara.getID_SVOLGIMENTO())) {
				int i=1;
				...
				for(AggiudicatarioBean ab : saBean.getAggiudicatari()) {
					...
					if(ab.getImpAggiudicatario()!=null)
						...
						} else if(ab.getRuolo()!=null && 
								  ab.getRuolo().equals(Costanti.MANDANTE) && 
								  ab.getImpAggiudicatario()!=null && 
								  ab.getImpAggiudicatario().doubleValue()>0)
                               mEccezioni.addValidationErrElemento(Messaggi.SIMOG_VALIDAZIONE_255b, i);
					i++;
				}
						if (sum.doubleValue() != saBean.getAggiudicazione().getImportoAggiudicazione().doubleValue()) {
							mEccezioni.addValidationField("label_SommaImporti");
					...
					if (saBean.getAggiudicatari().get(0).getImpAggiudicatario() != null
						...
						mEccezioni.addValidationField("label_ImportoAggiudicatario");
					...
					if (saBean.getAggiudicatari().get(0).getPercAumentoAggiudicatario() != null
						...
						mEccezioni.addValidationField("label_OffertaAumentoAggiudicatario");
						mEccezioni.addValidationErr(Messaggi.SIMOG_VALIDAZIONE_255.replace("$1",
           ...
           if(resp.getSoggettoPartecipante() != null && resp.getSoggettoResponsabile() == null)
			...
			try{
				if(isEmptyOrZero(resp.getIdRuolo()))
					throw new Exception();
				else if(!isRuoloValido(resp.getIdRuolo(), PSBD.SEZIONE_RE,resp.getDataInizioScheda()))
					throw new Exception();
			}catch (Exception e) {
		...
		for(AggiudicatarioBean aggiud: aggiudicatari){	
			...
			*/
			try{
				...
				if(!isEmpty(aggiud.getRuolo())){
					if(!this.validaRuolo(aggiud.getRuolo())){
						mEccezioni.addValidationField("label_CampoRuolo");
						mEccezioni.addValidationErr(Messaggi.SIMOG_VALIDAZIONE_108.replace("$1","campo Ruolo"));
					}
				}
			}catch (Exception e) {
       ...
       if( SimogFlags.isOrganiCostActive()){
		...
		if(SimogFlags.is30220Active() && SimogFlags.isVarAnagActive() && aggiudicazione.isValidaVariazione()){
			...
			if (aggiudicazione.getIdMotivoVarCO() == null) {
				mEccezioni.addValidationField("label_MotivazioneAnagrafica");
				mEccezioni.addValidationErr(
				...
				try{
					...
					if (SimogFlags.is3043Active()
							&& (modoRealizz != Costanti.MODOREAL_CONCESSIONE_LAVORI
									&& modoRealizz != Costanti.MODOREAL_FINANZA_DI_PROGETTO)
...
//			mEccezioni.addValidationField("label_CodiceNUTS");
//			mEccezioni.addValidationErr(
...
//		else 
			...
			if( SimogFlags.isOrganiCostActive() == false || isOrgano == false){
				...
				mEccezioni.addValidationField("label_CodiceISTAT");
				mEccezioni.addValidationErr(
		...
		// se non è attiva la nuova logica valido il vecchio codice cup di
		// aggiudicazione
      if(!SimogFlags.is3031_RFWEBGL02Active()
		...
		if(SimogFlags.is30216Active()){
			if(!isEmpty(aggiudicazione.getImportoAttuazioneSicurezza())){
				try{
                    ...
                    mEccezioni.addValidationField("label_ImportoAttuazioneSicurezza");
		...
		try{
			...
			mEccezioni.addValidationField("label_ImportoComplessivoAppalto");
			mEccezioni.addValidationErr(
					Messaggi.SIMOG_VALIDAZIONE_211.replace("$1", "l'importo complessivo dell'appalto"));
		}
		...
		if(!isEmpty(aggiudicazione.getImportoDisposizione())){
			...
			try{
				...
				mEccezioni.addValidationField("label_ImportoTotaleDisposizione");
				mEccezioni.addValidationErr(
						Messaggi.SIMOG_VALIDAZIONE_111.replace("$1", "l'importo totale delle somme a disposizione"));
			}
			try{
		...
		//TICKET ALM - 3.04.2 NG #2847 - scelta contraente
		...
		try {
            ...
            g = new GaraManager(connection,logger).getGara(l.getId_Gara());
		...
		try {
			dataCreazioneTime = sdf.parse(g.getData_creazione().trim()).getTime();
		...
		if(dataCreazioneTime < SimogProperties.getInstance().getDataAttivazione3042Timestamp()) {
			try{
				...
				mEccezioni.addValidationField("label_ProceduraSceltaContraente");
				mEccezioni.addValidationErr(
						Messaggi.SIMOG_VALIDAZIONE_104.replace("$1", "Procedura di scelta contraente"));
			}
		}
		...
		try{
			...
			mEccezioni.addValidationField("label_RicorsoAstaElettronica");
		...
		if(!isEmpty(aggiudicazione.getPercOffAumento())){
			if (!isPositive(aggiudicazione.getPercOffAumento())) {
				mEccezioni.addValidationField("label_OffertAumenti");
		...
		if (isEmpty(aggiudicazione.getImportoAggiudicazione())) {
			mEccezioni.addValidationField("label_AggiudicazioneAffidamento");
			...
			try{
				...
				mEccezioni.addValidationField("label_ValoreImportoAggiudicazione");
		...
		if(isEmptyOrZero(aggiudicazione.getDataVerbaleAggiudicazione())){
			mEccezioni.addValidationField("label_dataAggiudicazione");
			...
			try{
				...
				// PP 20110919 passa a warning Piccinini
				// mEccezioni.addValidationErr(Messaggi.SIMOG_VALIDAZIONE_161.replace("$1",
				// "Data di aggiudicazione definitiva o definizione della procedura
				// negoziata"));
		...
		// PP data stipula facoltativa, se valorizzata deve essere una data valida
		if(!isEmpty(aggiudicazione.getDataStipula())){
			try{
				if(!isDate(aggiudicazione.getDataStipula()))
					throw new Exception();
			} catch (Exception e) {
				mEccezioni.addValidationField("label_dataStipulaContratto");
				mEccezioni.addValidationErr(Messaggi.SIMOG_VALIDAZIONE_106.replace("$1", "Data stipula del contratto"));
			}
		}	
		
		// PP durata contrattuale facoltativo e numerico, non faccio controlli
	}
	...
	private void validaDatiComuni(InfoComuniBean infoComuni, float importo_lotto) {
		...
		if (isEmptyOrZero(infoComuni.getTipoContratto())) {
			mEccezioni.addValidationField("label_categoriaPrevalenteAppalto");
			mEccezioni.addValidationErr(
					Messaggi.SIMOG_VALIDAZIONE_104.replace("$1", "Categoria prevalente nell'appalto"));			
		}
...
*/		
		...
		// FIXMATO: PP abilitare quando si attiva il nuovo flusso per i sottosoglia ed
		// esclusi
		try {
			if (!checkTipoFlusso(infoComuni)) {
				mEccezioni.addValidationField("label_tipologiaAcquisizioneNonValida");
				mEccezioni.addValidationErr(Messaggi.SIMOG_VALIDAZIONE_188);
			}
		} catch (SQLException e1) {
			mEccezioni.addValidationField("label_lottoFlussoAcquisizione");
			mEccezioni.addValidationErr(
...
//			else 
		...
		if(isYFlag(infoComuni.getFlagSAAgente())){
			...
			try {
				...
				mEccezioni.addValidationField("label_codiceFiscaleSA");
...
//			}	
			try {
				...
				mEccezioni.addValidationField("label_denominazioneSA");
				mEccezioni.addValidationErr(Messaggi.SIMOG_VALIDAZIONE_112.replace("$1",
			...
			if (!isEmptyOrZero(infoComuni.getDenAmmAgente())) {
				mEccezioni.addValidationField("label_denominazoneSoggettoSA");
				mEccezioni.addValidationErr(
		...
		if(isEmpty(infoComuni.getEsitoProcedura())){
			mEccezioni.addValidationField("label_EsitoProceduraStatoAttuale");
			mEccezioni.addValidationErr(
	...
	private Boolean tipologiaSaValida(long idTipologia,Object o) throws SQLException{
		...
		return adb.getTipologica(TIPOLOGIA_SA.TABLE_NAME, TIPOLOGIA_SA.ID_TIPOLOGIA_SA, TIPOLOGIA_SA.DESCRIZIONE,
...
//		double impCalcolato = getSubtotale1(l, s, f,progettazione) * (tipo == 0 ? (1+temp/100) : tipo == 1 ?(1-temp/100) : 1) 
...
//	}
	...
	public boolean validaCondizioni(List<CondizioneAggBean> condizioni){
		...
		for(CondizioneAggBean condizione : condizioni){
			...
			esito_locale = super.validaTipologica(CONDIZIONI.TABLE_NAME, CONDIZIONI.ID_CONDIZIONE,
					CONDIZIONI.DESCRIZIONE, CONDIZIONI.T_DATA_FINE_VALIDITA, condizione.getDataInizioCond(), id);
			if(!esito_locale){
	 ...
	 *public boolean obbligoCondizioniCriteriAggiudicazione(List<CondizioneAggBean> condizioni){
		boolean esito_locale = true;
		for(CondizioneAggBean condizione : condizioni){
			if(condizione.getIdCondizione()==Costanti.DLGS_163_ART_57_C2LB ||condizione.getIdCondizione()==Costanti.DLGS_163_ART_57_C3LBF ||
			   condizione.getIdCondizione()==Costanti.DLGS_163_ART_57_C3LDF || condizione.getIdCondizione()==Costanti.DLGS_163_ART_57_C5LASA1 ||
			   condizione.getIdCondizione()==Costanti.DLGS_163_ART_57_C5LASA2 || condizione.getIdCondizione()==Costanti.DLGS_163_ART_57_C5LB ||
			   condizione.getIdCondizione()==Costanti.DLGS_163_ART_221_C1LC || condizione.getIdCondizione()==Costanti.DLGS_163_ART_221_C1LE ||
			   condizione.getIdCondizione()==Costanti.DLGS_163_ART_221_C1LF || condizione.getIdCondizione()==Costanti.DLGS_163_ART_221_C1LG ||
			   condizione.getIdCondizione()==Costanti.DLGS_163_ART_221_C1LJ || condizione.getIdCondizione()==Costanti.DLGS_163_ART_221_C1LK ||
